#include <16f887.h>
#fuses INTRC_IO, NOWDT, PROTECT, NOLVP, MCLR, NOBROWNOUT
#use delay(INTERNAL=4000000)
#use rs232(baud=9600, xmit=PIN_C6, rcv=PIN_C7)

#include <lcd.c>

#define LED1 PIN_B0    // Definimos el primer LED en el pin B0
#define Foco PIN_B1    // Definimos el Foco en el pin B1
#define PIR_SENSOR PIN_A0  // Pin para el sensor PIR

void main() {
    char comando;  // Variable para almacenar el comando recibido vía serie
    lcd_init();  
    setup_adc_ports(NO_ANALOGS);  
    set_tris_b(0b11111100);  // Configurar RB0 y RB1 como salidas; el resto como entradas
    set_tris_c(0b11111111);  // Configurar todo el puerto C como entradas, para leer el PIR

    while (true) {
         //Interacción con interfaz
        if (kbhit()) {  
            comando = getc();  // Leer el comando recibido
            // Instrucciones para cada comando recibido
            switch (comando) {
                // Apagar LED 1
                case '2':
                    output_low(LED1); 
                    lcd_gotoxy(1, 1);
                    printf(lcd_putc, "LED1: -Apagado-");
                    printf("LED1 Apagado\r\n");
                    break;

                // Encender LED 1
                case '1':
                    output_high(LED1);  
                    lcd_gotoxy(1, 1);
                    printf(lcd_putc, "LED1: Encendido");
                    printf("LED1 Encendido\r\n");
                    break;

                // Encender Foco (Comando por terminal)
                case '3':
                    output_high(Foco);  
                    lcd_gotoxy(1, 2);
                    printf(lcd_putc, "Foco: Encendido");
                    printf("Foco Encendido\r\n");
                    break;

                // Apagar Foco (Comando por terminal)
                case '4':
                    output_low(Foco);  
                    lcd_gotoxy(1, 2);
                    printf(lcd_putc, "Foco: -Apagado-");
                    printf("Foco Apagado\r\n");
                    break;
            }
        }

        // Leer el estado del sensor PIR y encender o apagar Foco
        if (input(PIR_SENSOR)) {  // Si el sensor PIR detecta movimiento (HIGH)
            if (comando != '3' && comando != '4') {
               output_high(Foco);
               lcd_gotoxy(1, 2);
               printf(lcd_putc, "Foco: Encendido");
               printf("Foco Encendido\r\n");
            }
        } else {  // Si no hay movimiento (LOW)
            // Solo apagar el Foco si no está siendo controlado por el comando
            if (comando != '3' && comando != '4') {
                output_low(Foco);
                lcd_gotoxy(1, 2);
                printf(lcd_putc, "Foco: -Apagado-");
                printf("Foco Apagado\r\n");
            }
        }

        delay_ms(400);  // Esperar un poco para no saturar el microcontrolador
    }
}

